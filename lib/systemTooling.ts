...
const PF2_XP_CONFIG: XpSystemConfig = {
  defaults: {
    levels: '20',
    baseXp: '1000',
    growthFactor: '1',
  ...
  labels: {
    ...
    encountersPerLevel: 'Expected Encounters per Level',
    notes: 'GM Notes',
    notesPlaceholder: 'Treasure spike at 5, archetype beat at 8, campaign chapter reset at 12...',
    rerollButton: 'Reroll Advancement Ideas',
    pacingSummary: 'Advancement Summary',
    levelingPreview: 'Level Preview',
    milestoneSuggestions: 'Level Beats',
    practicalAdvice: 'GM Advice',
    optionalPacingVariants: 'Optional Advancement Variants',
    milestoneModeCopy: 'Milestone mode is active. Advancement is tied to chapter wins and scenario beats instead of exact XP totals.',
  },
  presetLabels: {
    slow: 'adventure path',
    standard: 'standard',
  ...
  pacingAssessment: {
    default: 'Steady level-based pacing with clear advancement intervals.',
    fast: 'Fast advancement that pushes the party through low-level content quickly.',
    slow: 'A slower track that gives room for denser scenarios, downtime, and treasure pacing.',
    milestone: 'Milestone pacing lets you advance by chapters while still planning around level bands.',
  },
  advice: {
    milestoneMode: 'Milestone mode works well when you still want clear level bands but do not want to track exact XP scene by scene.',
    xpMode: 'Tracked XP supports a very steady advancement rhythm when encounter difficulty is planned intentionally.',
    singleSession: 'Very few encounters per session can make level pacing feel slower than the rest of the campaign structure.',
    highEncounterCount: 'High encounters-per-level can overextend a level band unless treasure and new options stay exciting.',
    steepCurve: 'A slow-burn curve is useful only if you intentionally want later levels to take longer than the standard cadence.',
    linearCurve: 'A flat per-level XP target keeps advancement predictable and easy to communicate to the table.',
    default: 'This setup should support a steady level-based campaign without much pacing friction.',
  },
  pacingVariantPool: [
    'Use chapter-end advancement so the table still feels level-based even when exact XP is not tracked live.',
    'Pair level increases with treasure refreshes so character growth and item growth feel synchronized.',
    'Treat one scenario site or dungeon wing as a complete advancement chapter.',
    'Use short catch-up milestones for replacement characters so party math stays close.',
    'Bundle social, exploration, and investigation wins into the same XP economy as combat encounters.',
    'Let downtime accomplishments close the gap on a slow level without adding filler fights.',
    'Reserve one advancement beat for a faction promotion, archetype turn, or campaign pivot.',
    'Use a flatter track for teaching campaigns and a slower track for mastery-focused tables.',
    'Preview new level expectations one session early so encounter prep and treasure handoffs stay smooth.',
    'When the campaign broadens in scale, increase complexity before increasing raw encounter volume.',
  ],
  milestoneBase: (levels) => [
    'Level 2: reinforce the party’s tactical identity and early feat choices.',
    `Level ${Math.max(5, Math.floor(levels * 0.25))}: widen encounter complexity and tighten treasure pacing.`,
    `Level ${Math.max(10, Math.floor(levels * 0.5))}: shift the campaign into a stronger mid-level mastery band.`,
    `Level ${Math.max(17, levels - 2)}: reserve for endgame capability, large threats, and final chapter structure.`,
  ],
  milestoneVariants: (levels) => [
    `Level ${Math.max(3, Math.floor(levels * 0.15))}: spotlight a class-defining build choice or archetype beat.`,
    `Level ${Math.max(6, Math.floor(levels * 0.35))}: introduce stronger action-taxing enemies and terrain pressure.`,
    `Level ${Math.max(9, Math.floor(levels * 0.5))}: let the campaign widen from local pressure into regional stakes.`,
    `Level ${Math.max(13, Math.floor(levels * 0.7))}: add scenario complexity before simply adding harder numbers.`,
    `Level ${Math.max(18, levels - 1)}: preview finale expectations with one controlled high-pressure encounter.`,
  ],
};
...
const PF2_REWARD_CONFIG: RewardSystemConfig = {
  defaults: {
    playerLevel: '5',
    enemyTier: '1',
    rewardType: 'gear',
  ...
  labels: {
    playerLevel: 'Party Level',
    enemyTier: 'Encounter Tier',
    rewardType: 'Treasure Type',
    rarity: 'Rarity',
    rewardSource: 'Treasure Source',
    rewardTheme: 'Treasure Theme',
    bundleStyle: 'Parcel Size',
    prepNotes: 'GM Notes',
    prepNotesPlaceholder: 'Treasure parcel for the next level band, consumable refill, item upgrade beat...',
    rerollButton: 'Reroll Treasure Parcel',
    rewardSummary: 'Treasure Parcel Summary',
    sourceGuidance: 'Parcel Guidance',
    practicalAdvice: 'Treasure Pacing Advice',
    encounterHooks: 'Scenario Hook Ideas',
    featuredItem: 'Featured parcel item',
    itemDetail: 'Parcel detail',
    statLine: 'Use note',
    bonusItem: 'Bonus parcel piece',
    currencyValue: 'Treasure value',
  },
  rarityLabels: SHARED_RARITY_LABELS,
  rewardTypeLabels: {
    gear: 'permanent item',
    gold: 'currency bundle',
    consumable: 'consumable tool',
    material: 'crafting material',
  },
  rewardSourceLabels: {
    boss: 'boss cache',
    chest: 'site cache',
    quest: 'scenario reward',
    vendor: 'market stock',
    faction: 'patron support',
  },
  rewardThemeLabels: {
    arcane: 'arcane',
    divine: 'sacred',
    cursed: 'shadowed',
    martial: 'battle-ready',
    wilderness: 'wild',
    noble: 'courtly',
  },
  bundleStyleLabels: {
    lean: 'light parcel',
    balanced: 'standard parcel',
    generous: 'high-value parcel',
  },
  flavorNotes: {
    boss: 'Boss caches should justify the threat jump and help the party feel ready for the next level band.',
    chest: 'Site caches should feel placed with intent and support the current scenario arc.',
    quest: 'Scenario rewards should reinforce momentum and shore up what the party will need next.',
    vendor: 'Market stock should feel dependable, targeted, and level-appropriate.',
    faction: 'Patron support should reinforce alliances and campaign logistics, not just add loose value.',
  },
  advice: {
    boss: 'Boss treasure works best when it either fills a party gap or sharpens an existing build plan.',
    rareGold: 'Pure value can feel abstract. Pair high-value payouts with a specific item path or next purchase target.',
    material: 'Crafting materials land best when the party already has a likely use for them.',
    vendor: 'Market options should be practical and level-aware rather than overly random.',
    generous: 'High-value parcels should be saved for major scenario finishes, bosses, or a level-band transition.',
    default: 'This parcel should fit cleanly into a level-based treasure flow.',
  },
  hookPool: [
    'The parcel points toward the next site or scenario objective.',
    'A rival claimant identifies the item and wants it back.',
    'One item can be upgraded if the party follows a lead tied to this reward.',
    'A patron notices who carries the reward and changes their offer accordingly.',
    'The parcel completes part of a larger set the party has only partly uncovered.',
    'The treasure reveals who was funding the current threat behind the scenes.',
  ],
  bonusPools: {
    lean: ['small coin add-on', 'useful consumable backup', 'minor crafting packet'],
    balanced: ['extra consumable', 'small permanent upgrade lead', 'crafting supplement'],
    generous: ['secondary permanent item', 'larger coin bundle', 'rare upgrade material'],
  },
  rewardSummary: ({ rarity, rewardTheme, rewardType, rewardSource }) =>
    `${rarity} ${rewardTheme} ${rewardType} parcel from a ${rewardSource}.`,
  itemPools: {
    arcane: {
      gear: ['Spelletched Staff', 'Resonant Focus', 'Runic Handwraps', 'Aether Lens'],
      gold: ['Arcane Society Stipend', 'Vault Coin Tube', 'Research Grant Purse', 'Conclave Reserve'],
      consumable: ['Spellguard Elixir', 'Blink Dust Vial', 'Arcane Scroll Tube', 'Focus Tonic'],
      material: ['Resonant Crystal', 'Spellthread Coil', 'Aether Sand', 'Runic Ink Packet'],
    },
    divine: {
      gear: ['Consecrated Buckler', 'Votive Charm', 'Sunlit Blade', 'Pilgrims Reliquary'],
      gold: ['Temple Support Purse', 'Sanctuary Roll', 'Pilgrim Fund', 'Relic Repair Grant'],
      consumable: ['Sanctified Water', 'Mercy Elixir', 'Prayer Incense Bundle', 'Dawnlight Capsule'],
      material: ['Blessed Resin', 'Silver Filament', 'Halo Dust', 'Consecrated Ash'],
    },
    cursed: {
      gear: ['Shadowbound Ring', 'Grim Knife', 'Wraiththread Cloak', 'Oathscar Pendant'],
      gold: ['Black Ledger Purse', 'Funeral Coin Wrap', 'Forbidden Tribute', 'Night Debt Roll'],
      consumable: ['Hex Antidote', 'Ash Smoke Capsule', 'Rot Elixir', 'Darkward Salve'],
      material: ['Shadow Glass', 'Witchbone Dust', 'Night Iron', 'Mourning Wax'],
    },
    martial: {
      gear: ['Potency-Etched Blade', 'Guard Captains Shield', 'Skirmisher Harness', 'Field Commanders Pike'],
      gold: ['Quartermasters Purse', 'Campaign Reserve', 'Patrol Bonus Roll', 'War Fund Packet'],
      consumable: ['Battle Elixir', 'Sharpening Kit', 'Smoke Capsule', 'Recovery Draught'],
      material: ['Forged Steel Ingot', 'Rune Etching Kit', 'Reinforced Leather', 'Siege Twine Spool'],
    },
    wilderness: {
      gear: ['Trailblazers Bow', 'Thornwoven Cloak', 'Rootward Hatchet', 'Wardens Hood'],
      gold: ['Frontier Supply Purse', 'Hunters Cache', 'Path Wardens Roll', 'Wildland Trade Coin'],
      consumable: ['Predator Ward Salve', 'Stormleaf Tea', 'Field Antidote', 'Trail Ration Pack'],
      material: ['Spirit Bark', 'Skyfeather Bundle', 'Verdant Ore', 'Beast Tendon Coil'],
    },
    noble: {
      gear: ['Embassy Blade', 'Court Signet', 'Regents Cloakpin', 'Duelists Bracers'],
      gold: ['House Dividend Purse', 'Council Stipend', 'Court Reward Roll', 'Royal Credit Packet'],
      consumable: ['Composure Draught', 'Courtly Remedy', 'Luxury Antitoxin', 'Banquet Reserve Flask'],
      material: ['Silk Spool', 'Lacquer Plate', 'Silver Seal Kit', 'Pearl Dust Packet'],
    },
  },
  itemDetail: ({ rewardType, rewardTheme, rarity, itemName }) => ({
    description: `${itemName} is a ${rarity} ${rewardTheme} parcel item meant to feel level-appropriate and purposeful.`,
    statLine:
      rewardType === 'gear'
        ? 'Treat as a level-appropriate permanent item with one clear tactical niche.'
        : rewardType === 'consumable'
          ? 'Single-use item that fills a likely short-term party need.'
          : rewardType === 'material'
            ? 'Useful for upgrades, crafting, or supporting the next item progression beat.'
            : 'Use as treasure value or a clean payout toward the next planned purchase.',
  }),
};
