...
create index if not exists idx_saved_projects_campaign_id on public.saved_projects(campaign_id);
...
drop policy if exists "Users can manage their own saved projects" on public.saved_projects;
...
grant execute on function public.downgrade_to_free_and_trim_projects(uuid) to authenticated;
